...
## v5 文章品質改善
- Aozoraコーパスを文書単位に分割して学習し、制御文字を除去。
- AozoraとDiscord会話で共通のTokenizerを再構築。
- Discordのinputだけでなくinput→responseを会話形式で学習。
- モデル容量を拡張し、生成温度・反復抑制を文章品質重視に調整。
- 生成結果の壊れた文字列・重複・制御記号を除去。
- 固定の「なるほど。」フォールバックを廃止。
